    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden relative z-10 border border-slate-800/20">
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-10 text-center relative">
          ...
          <div className="absolute top-4 right-4 bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border border-cyan-500/20">
        ...
        <div className="p-10 space-y-6">
          ...
          <div className="space-y-4">
            <p className="text-xs font-bold text-slate-400 uppercase text-center mb-4">Select Role Profile (Demo Mode)</p>
            <div className="grid grid-cols-1 gap-3">
              {MOCK_USERS.map((user) => (
                ...
                >
                  <div className="w-10 h-10 bg-white group-hover:bg-cyan-500/20 rounded-lg flex items-center justify-center text-slate-600 group-hover:text-white transition-colors">
                    👤
                  </div>
                  <div>
                    <p className="text-sm font-bold text-slate-800 group-hover:text-white leading-none mb-1">{user.name}</p>
                    <p className="text-[10px] text-slate-500 group-hover:text-cyan-100 font-medium">{user.role}</p>
                  </div>
          ...
          <div className="pt-8 border-t border-slate-100 text-center">
            <p className="text-[10px] text-slate-400 leading-relaxed uppercase tracking-widest font-bold mb-4">Administrative Support</p>
            <div className="flex justify-center gap-4">
              <a href="#" className="text-xs font-medium text-slate-500 hover:text-cyan-600 transition-colors">Help Desk</a>
              <span className="text-slate-200">|</span>
              <a href="#" className="text-xs font-medium text-slate-500 hover:text-cyan-600 transition-colors">Digital Portal</a>
              <span className="text-slate-200">|</span>
              <a href="#" className="text-xs font-medium text-slate-500 hover:text-cyan-600 transition-colors">Policy</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <p className="mt-8 text-slate-500 text-[10px] font-bold uppercase tracking-[0.2em] relative z-10">
        © 2024 Labour Department • Telangana State • Digital Governance Initiative
      </p>
    </div>
  );
};

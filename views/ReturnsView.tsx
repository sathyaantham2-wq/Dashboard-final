  ...
  const ActCaseTable = ({ title }: { title: string }) => (
    <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 mb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <InputGroup label="Pending Start" /><InputGroup label="Filed" /><InputGroup label="Disposed" /><InputGroup label="Pending End" /><InputGroup label="Benefitted" /><InputGroup label="Reserved" />
      </div>
    </div>
  );
